...
export default function MzdyPage() {
  ...
  // Získanie pekného názvu mesiaca pre nadpis v PDF
  const aktualnyNazovMesiaca = zoznamMesiacov.find(m => m.hodnota === filterMesiac)?.nazov || filterMesiac;
  ...
  useEffect(() => { if (jeOdomknute) nacitaj() }, [filterMesiac, jeOdomknute])

  function zmenaStavuStavby(stavba: string, hodnota: string) {
    const kluc = `${filterMesiac}_${stavba}`
  ...
  Object.entries(stavbyData).forEach(([stavba, hodiny]) => {
    const stav = stavyStavieb[`${filterMesiac}_${stavba}`] || 'Nezaplatená'
  ...
  // Funkcia, ktorá po kliknutí vyvolá tlačové okno prehliadača
          ...
          <tbody>
            {zoznam.length === 0 ? (
              ...
              zoznam.map(([stavba, hodiny]) => {
                const aktualnyStav = stavyStavieb[`${filterMesiac}_${stavba}`] || 'Nezaplatená'
                ...
                return (
                  <tr key={stavba} style={{ borderBottom: '1px solid #f3f4f6' }}>
                    ...
                    {/* Pri tlači sa namiesto rozbaľovacieho selectu ukáže len text stavu */}
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#fafafa', padding: '40px 20px', display: 'flex', justifyContent: 'center' }}>
      ...
      {/* ŠPECIÁLNY CSS ŠTÝL PRE TLAČ PDF */}
      <style>{`
        @media print {
      ...
      <div className="hlavny-kontajner" style={{ width: '100%', maxWidth: '900px', backgroundColor: 'white', padding: '40px', borderRadius: '20px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)' }}>
        ...
        {/* NAVIGAČNÉ MENU (skryje sa v PDF) */}
        ...
        {/* HLAVIČKA DOKUMENTU PRE PDF (ukáže sa iba pri tlači/PDF) */}
        ...
        {/* TOP FILTER A TLAČIDLO PRE EXPORT */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px', flexWrap: 'wrap', gap: '20px', backgroundColor: '#f9fafb', padding: '15px 20px', borderRadius: '12px' }}>
          <div>
            <span className="skryt-pri-tlaci" style={{ fontWeight: '600', color: '#374151', marginRight: '15px' }}>Prehľad za obdobie:</span>
          ...
          {/* NOVÉ TLAČIDLO PRE EXPORT DO PDF */}
          <button 
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {Object.keys(zamestnanciHodiny).length === 0 ? (
                <tr><td colSpan={4} style={{ padding: '30px 0', color: '#666', textAlign: 'center' }}>V zvolenom mesiaci neevidujeme žiadnu prácu zamestnancov.</td></tr>
              ) : (
